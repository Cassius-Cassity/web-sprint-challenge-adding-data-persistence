...
1. Explain the difference between Relational Databases and SQL.
  relational databases is a database software while sql is a language used to navigate the database itself. Relational databases contain the data itself while sql is a query language that allows us to manipulate a database. 
2. Why do tables need a Primary Key?
  they provide a unique value that can identify a specific row in a table. This is especially useful when reffering to the model.js because it would make it more difficult to reference a specific row. 
3. What is the name given to a table column that references the Primary Key on another table?
  the name given to a table column that references another table is called a foreign key.
4. What do we need in order to have a _many to many_ relationship between two tables?
  One way that we can deal with a many to many relationship is by establishing a junction table. 